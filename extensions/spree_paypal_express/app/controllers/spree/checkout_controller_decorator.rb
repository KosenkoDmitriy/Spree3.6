module Spree
  CheckoutController.class_eval do
    ...
    def paypal_finish
      ...
      ppx_auth_response = gateway.send(method, (@order.total.round(2) * 100).to_i, opts)
      ...
      if ppx_auth_response.success?
        ...
        @order.finalize!
        flash[:notice] = Spree.t(:thank_you_email_confirmation)
    ...
    def order_opts(order, payment_method_id, stage)
      # [KYAN] "items"  are not actually used at the moment
      items = order.line_items.map do |item|
        price = (item.price.to_f.round(2) * 100).to_i # convert for gateway
      ...
      credits = order.adjustments.eligible.map do |credit|
        if credit.amount < 0.00
          { :name        => credit.label,
            ...
            :sku         => credit.id,
            :quantity    => 1,
            :amount      => (credit.amount.round(2) * 100).to_i }
        end
      end
      ...
      if payment_method.preferred_cart_checkout and (order.shipping_method.blank? or order.ship_total == 0)
        ...
        order_total    = (order.total.round(2) * 100 + (shipping_cost)).to_i
        shipping_total = (shipping_cost).to_i
      else
        order_total    = (order.total.round(2) * 100).to_i
        shipping_total = (order.ship_total.round(2) * 100).to_i
      end
      ...
      opts = { :return_url        => paypal_confirm_order_checkout_url(order, :payment_method_id => payment_method_id),
               ...
               :custom            => order.number,
               # [KYAN] Do NOT include the items descriptions, this is due to the item totals not rounding nicely
               # And Paypal trys to check that all the totals add up correctly
               #:items             => items,
               #:subtotal          => ((order.item_total.round(2) * 100) + credits_total).to_i,
               #:tax               => (order.tax_total.round(2) * 100).to_i,
               #:shipping          => shipping_total,
               :money             => (order.total.round(2) * 100).to_i,
               :max_amount        => (order.total.round(2) * 300).to_i}
    ...
    def all_opts(order, payment_method_id, stage=nil)
      ...
      if order.bill_address.present?
        ...
        opts[:address] = {
          ...
          :state => order.bill_address.state.nil? ? order.bill_address.state_name.to_s : order.bill_address.state.name,
    ...
    def payment_method
      payment_method_id = params[:payment_method_id] || params[:order][:payments_attributes].first[:payment_method_id]
