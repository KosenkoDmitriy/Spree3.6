...
class FindAddressHelper
  ...
  def self.find_address(email)
    #past = Order.order("created_at").where(:email => email).limit(5)
		previous = Order.order("completed_at DESC, created_at DESC").where(["email LIKE ? AND bill_address_id IS NOT NULL AND ship_address_id IS NOT NULL", email])
    previous.reverse!.pop
    bill_address , ship_address = nil , nil
    while !bill_address and order = previous.pop
