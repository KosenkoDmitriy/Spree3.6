...
CheckoutController.class_eval do
  ...
  def before_address
    #puts "BEFORE ADDRESS2 called user= #{@order}  #{@order.email}"
		previous_bill_address, previous_ship_address = FindAddressHelper.find_address(@order.email)
		
		@order.bill_address ||= previous_bill_address
		@order.ship_address ||= previous_bill_address
    # in case none found
